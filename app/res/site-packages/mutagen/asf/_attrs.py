...
from mutagen._compat import PY2, reraise, swap_to_string, text_type
...
class ASFBaseAttribute(object):
    ...
    def render_m(self, name):
        ...
        if self.TYPE == 2:
            ...
            data = self._render()
        return struct.pack("<HHHHI", 0, self.stream or 0, len(name), self.TYPE, len(data)) + name + data
    ...
    def render_ml(self, name):
        ...
        return struct.pack("<HHHHI", self.language or 0, self.stream or 0, len(name), self.TYPE, len(data),) + name + data
