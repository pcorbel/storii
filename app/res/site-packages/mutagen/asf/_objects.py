...
from mutagen._compat import izip, text_type, xrange
...
class BaseObject(object):
    ...
    def __repr__(self):
        return "<%s GUID=%s objects=%r>" % (type(self).__name__, bytes2guid(self.GUID), self.objects)
...
class HeaderObject(BaseObject):
    ...
    def render_full(self, asf, fileobj, available, padding_func):
        ...
        data = HeaderObject.GUID + struct.pack("<QL", len(data) + 30, num_objects) + b"\x01\x02" + data
    ...
    def render(self, asf):
        ...
        for obj in self.objects:
            ...
            if obj.GUID == PaddingObject.GUID:
                continue
            data += obj.render(asf)
        return self.GUID + struct.pack("<Q", 24 + 16 + 6 + len(data)) + b"\x11\xD2\xD3\xAB\xBA\xA9\xcf\x11" + b"\x8E\xE6\x00\xC0\x0C\x20\x53\x65" + b"\x06\x00" + struct.pack("<I", len(data)) + data
...
class MetadataObject(BaseObject):
    ...
    def parse(self, asf, data):
        ...
        pos = 2
        for i in xrange(num_attributes):
            (reserved, stream, name_length, value_type, value_length) = struct.unpack("<HHHHI", data[pos : pos + 12])
            pos += 12
            name = data[pos : pos + name_length]
            name = name.decode("utf-16-le").strip("\x00")
            pos += name_length
...
class MetadataLibraryObject(BaseObject):
    ...
    def parse(self, asf, data):
        ...
        pos = 2
        for i in xrange(num_attributes):
            (language, stream, name_length, value_type, value_length) = struct.unpack("<HHHHI", data[pos : pos + 12])
            pos += 12
            name = data[pos : pos + name_length]
            name = name.decode("utf-16-le").strip("\x00")
            pos += name_length
