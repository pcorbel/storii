...
from mutagen._util import DictMixin, MutagenError, cdata, convert_error, delete_bytes, get_size, loadfile, seek_end, total_ordering

from ._compat import PY2, PY3, cBytesIO, reraise, swap_to_string, text_type, xrange
...
def is_valid_apev2_key(key):
    ...
    return (2 <= len(key) <= 255) and (min(key) >= u" ") and (max(key) <= u"~") and (key not in [u"OggS", u"TAG", u"ID3", u"MP+"])
...
class APEv2(_CIDictProxy, Metadata):
    ...
    def save(self, filething=None):
        ...
        header += struct.pack("<4I", 2000, len(tags) + 32, num_tags, HAS_HEADER | IS_HEADER)
        header += b"\0" * 8
