...
from mutagen._util import MutagenError, convert_error, get_size, loadfile, resize_bytes
from mutagen.id3._util import BitPaddedInt

from ._compat import cBytesIO, chr_, endswith, xrange
...
class SeekPoint(tuple):
    ...
    def __new__(cls, first_sample, byte_offset, num_samples):
        return super(cls, SeekPoint).__new__(cls, (first_sample, byte_offset, num_samples))
...
class SeekTable(MetadataBlock):
    ...
    def load(self, data):
        ...
        while len(sp) == self.__SEEKPOINT_SIZE:
            self.seekpoints.append(SeekPoint(*struct.unpack(self.__SEEKPOINT_FORMAT, sp)))
            sp = data.tryread(self.__SEEKPOINT_SIZE)

    def write(self):
        f = cBytesIO()
        for seekpoint in self.seekpoints:
            packed = struct.pack(self.__SEEKPOINT_FORMAT, seekpoint.first_sample, seekpoint.byte_offset, seekpoint.num_samples)
            f.write(packed)
        return f.getvalue()
...
class CueSheetTrack(object):
    ...
    def __init__(self, track_number, start_offset, isrc="", type_=0, pre_emphasis=False):
    ...
    def __eq__(self, other):
        try:
            return self.track_number == other.track_number and self.start_offset == other.start_offset and self.isrc == other.isrc and self.type == other.type and self.pre_emphasis == other.pre_emphasis and self.indexes == other.indexes
        except (AttributeError, TypeError):
            return False
...
class CueSheet(MetadataBlock):
    ...
    def __eq__(self, other):
        try:
            return self.media_catalog_number == other.media_catalog_number and self.lead_in_samples == other.lead_in_samples and self.compact_disc == other.compact_disc and self.tracks == other.tracks
        except (AttributeError, TypeError):
            return False
    ...
    def load(self, data):
        ...
        media_catalog_number, lead_in_samples, flags, num_tracks = struct.unpack(self.__CUESHEET_FORMAT, header)
        ...
        for i in xrange(num_tracks):
            ...
            start_offset, track_number, isrc_padded, flags, num_indexes = struct.unpack(self.__CUESHEET_TRACK_FORMAT, track)
            ...
            for j in xrange(num_indexes):
                ...
                index_offset, index_number = struct.unpack(self.__CUESHEET_TRACKINDEX_FORMAT, index)
    ...
    def write(self):
        ...
        if self.compact_disc:
            flags |= 0x80
        packed = struct.pack(self.__CUESHEET_FORMAT, self.media_catalog_number, self.lead_in_samples, flags, len(self.tracks))
        f.write(packed)
        for track in self.tracks:
            ...
            if track.pre_emphasis:
                track_flags |= 0x40
            track_packed = struct.pack(self.__CUESHEET_TRACK_FORMAT, track.start_offset, track.track_number, track.isrc, track_flags, len(track.indexes),)
            f.write(track_packed)
            for index in track.indexes:
                index_packed = struct.pack(self.__CUESHEET_TRACKINDEX_FORMAT, index.index_offset, index.index_number)
                f.write(index_packed)
        return f.getvalue()

    def __repr__(self):
        return ("<%s media_catalog_number=%r, lead_in=%r, compact_disc=%r, " "tracks=%r>") % (type(self).__name__, self.media_catalog_number, self.lead_in_samples, self.compact_disc, self.tracks,)
...
class FLAC(mutagen.FileType):
    ...
    METADATA_BLOCKS = [StreamInfo, Padding, None, SeekTable, VCFLACDict, CueSheet, Picture]
    ...
    def delete(self, filething=None):
        ...
        if self.tags is not None:
            ...
            self.metadata_blocks[:] = [b for b in self.metadata_blocks if b.code != VCFLACDict.code or b is self.tags]
            self.tags.clear()
    ...
    def load(self, filething):
        ...
        if self.info.length:
            start = fileobj.tell()
            fileobj.seek(0, 2)
            self.info.bitrate = int(float(fileobj.tell() - start) * 8 / self.info.length)
        else:
            self.info.bitrate = 0
    ...
    def _save(self, filething, metadata_blocks, deleteid3, padding):
        ...
        data = MetadataBlock._writeblocks(metadata_blocks, available, content_size, padding)
        data_size = len(data)
