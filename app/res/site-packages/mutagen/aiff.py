...
from mutagen._util import MutagenError, convert_error, delete_bytes, insert_bytes, loadfile, resize_bytes
from mutagen.id3 import ID3
from mutagen.id3._util import ID3NoHeaderError
from mutagen.id3._util import error as ID3Error

from ._compat import endswith, reraise, text_type
...
class IFFChunk(object):
    ...
    def resize(self, new_data_size):
        ...
        resize_bytes(
            self._fileobj, self.data_size + self.padding(), new_data_size + padding, self.data_offset,
        )
...
class FormIFFChunk(IFFChunk):
    ...
    def insert_chunk(self, id_, data=None):
        ...
        self._fileobj.seek(next_offset)
        self._fileobj.write(pack(">4si", id_.ljust(4).encode("ascii"), data_size))
        self._fileobj.seek(next_offset)
        chunk = IFFChunk.parse(self._fileobj, self)
...
class AIFFInfo(StreamInfo):
    ...
    def pprint(self):
        return u"%d channel AIFF @ %d bps, %s Hz, %.2f seconds" % (self.channels, self.bitrate, self.sample_rate, self.length,)
