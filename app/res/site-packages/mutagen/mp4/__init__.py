...
from mutagen._compat import PY2, PY3, cBytesIO, chr_, iteritems, izip, reraise, string_types, text_type, xrange
from mutagen._constants import GENRES
from mutagen._util import DictProxy, MutagenError, cdata, convert_error, enum, get_size, hashable, insert_bytes, loadfile, resize_bytes
...
class MP4Cover(bytes):
    ...
    def __repr__(self):
        return "%s(%r, %r)" % (type(self).__name__, bytes(self), AtomDataType(self.imageformat))
...
class MP4FreeForm(bytes):
    ...
    def __eq__(self, other):
        ...
        return bytes(self) == bytes(other) and self.dataformat == other.dataformat and self.version == other.version

    def __ne__(self, other):
        return not self.__eq__(other)

    def __repr__(self):
        return "%s(%r, %r)" % (type(self).__name__, bytes(self), AtomDataType(self.dataformat))
    ...
    * 'gnre' -- ID3v1 genre. Not supported, use '\\xa9gen' instead.
    ...
    def __parse_data(self, atom, data):
        ...
        while pos < atom.length - 8:
            ...
            if name != b"data":
                raise MP4MetadataError("unexpected atom %r inside %r" % (name, atom.name))
    ...
    def __render_data(self, key, version, flags, value):
        return Atom.render(_key2name(key), b"".join([Atom.render(b"data", struct.pack(">2I", version << 24 | flags, 0) + data) for data in value]),)
    ...
    def __parse_freeform(self, atom, data):
        ...
        while pos < atom.length - 8:
            ...
            value.append(MP4FreeForm(data[pos + 16 : pos + length], dataformat=flags, version=version))
            pos += length
    ...
    def __render_freeform(self, key, value):
        ...
        for v in value:
            ...
            data += struct.pack(">I4s2I", len(v) + 16, b"data", version << 24 | flags, 0)
            data += v
    ...
    def __parse_pair(self, atom, data):
        key = _name2key(atom.name)
        values = [struct.unpack(">2H", d[2:6]) for version, flags, d in self.__parse_data(atom, data)]
    ...
    def __render_pair(self, key, value):
        ...
        for v in value:
            ...
            if 0 <= track < 1 << 16 and 0 <= total < 1 << 16:
                ...
                raise MP4MetadataValueError("invalid numeric pair %r" % ((track, total),))
    ...
    def __render_cover(self, key, value):
        ...
        for cover in value:
            try:
                ...
                imageformat = MP4Cover.FORMAT_JPEG
            atom_data.append(Atom.render(b"data", struct.pack(">2I", imageformat, 0) + cover))
        ...
        for version, flags, atom_data in self.__parse_data(atom, data):
            if implicit:
                if flags not in (AtomDataType.IMPLICIT, AtomDataType.UTF8):
                    raise MP4MetadataError("Unknown atom type %r for %r" % (flags, atom.name))
            else:
    ...
    def pprint(self):
        ...
        for key, value in sorted(iteritems(self)):
            ...
            if key == "covr":
                values.append(u"%s=%s" % (key, u", ".join([u"[%d bytes of data]" % len(data) for data in value])))
