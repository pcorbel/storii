...
from mutagen._util import BitReader, BitReaderError, MutagenError, convert_error, enum, intround
...
class MPEGFrame(object):
    ...
    def __init__(self, fileobj):
        ...
        if version == 1 or layer == 0 or sample_rate == 0x3 or bitrate == 0xF or bitrate == 0:
        ...
        frame_length = (((frame_size // 8 * self.bitrate) // self.sample_rate) + padding) * slot
        ...
        try:
            ...
            if xing.frames != -1:
                ...
                if xing.bytes != -1 and samples > 0:
                    ...
                    self.bitrate = intround((audio_bytes * 8 * self.sample_rate) / float(samples))
                if lame is not None:
...
class MPEGInfo(StreamInfo):
    ...
    def pprint(self):
        ...
        s = u"MPEG %s layer %d, %d bps (%s), %s Hz, %d chn, %.2f seconds" % (self.version, self.layer, self.bitrate, info, self.sample_rate, self.channels, self.length,)
        if self.sketchy:
            s += u" (sketchy)"
        return s
...
class MP3(ID3FileType):
    ...
    def score(filename, fileobj, header_data):
        ...
        return header_data.startswith(b"ID3") * 2 + endswith(filename, b".mp3") + endswith(filename, b".mp2") + endswith(filename, b".mpg") + endswith(filename, b".mpeg")
