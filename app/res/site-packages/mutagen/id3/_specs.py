...
from .._compat import PY3, chr_, string_types, swap_to_string, text_type, xrange
from .._util import cdata, decode_terminated, encode_endian, enum, flags, intround, izip, total_ordering
...
class EncodingSpec(ByteSpec):
    ...
    def read(self, header, frame, data):
        ...
        if enc not in (Encoding.LATIN1, Encoding.UTF16, Encoding.UTF16BE, Encoding.UTF8):
    ...
    def validate(self, frame, value):
        if value is None:
            raise TypeError
        if value not in (Encoding.LATIN1, Encoding.UTF16, Encoding.UTF16BE, Encoding.UTF8):
...
class RVASpec(Spec):
    ...
    def write(self, config, frame, values):
        if len(values) < 2 or len(values) > self._max_values:
            raise SpecError("at least two volume change values required, max %d" % self._max_values)
        ...
        byte_values = [BitPaddedInt.to_str(v, bits=8, width=-1, minwidth=2) for v in values]
    ...
    def validate(self, frame, value):
        ...
        if isinstance(value, list):
            if len(self.specs) == 1:
                ...
                return [[s.validate(frame, v) for (v, s) in izip(val, self.specs)] for val in value]
        raise ValueError("Invalid MultiSpec data: %r" % value)

    def _validate23(self, frame, value, **kwargs):
        if len(self.specs) != 1:
            return [[s._validate23(frame, v, **kwargs) for (v, s) in izip(val, self.specs)] for val in value]
...
class TimeStampSpec(EncodedTextSpec):
    ...
    def write(self, config, frame, data):
        return super(TimeStampSpec, self).write(config, frame, data.text.replace(" ", "T"))
...
class ChannelSpec(ByteSpec):
    (OTHER, MASTER, FRONTRIGHT, FRONTLEFT, BACKRIGHT, BACKLEFT, FRONTCENTRE, BACKCENTRE, SUBWOOFER) = xrange(9)
...
class VolumeAdjustmentsSpec(Spec):
    ...
    def write(self, config, frame, value):
        value.sort()
        return b"".join(struct.pack(">Hh", int(freq * 2), int(adj * 512)) for (freq, adj) in value)
