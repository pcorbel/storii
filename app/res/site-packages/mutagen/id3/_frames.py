...
from .._compat import iteritems, itervalues, izip, string_types, swap_to_string, text_type
from ._specs import ASPIIndexSpec, BinaryDataSpec, ByteSpec, ChannelSpec, CTOCFlagsSpec, EncodedNumericPartTextSpec, EncodedNumericTextSpec, EncodedTextSpec, Encoding, EncodingSpec, FrameIDSpec, ID3FramesSpec, IntegerSpec, KeyEventSpec, Latin1TextListSpec, Latin1TextSpec, MultiSpec, PictureTypeSpec, RVASpec, SizedIntegerSpec, SpecError, StringSpec, SynchronizedTextSpec, TimeStampSpec, VolumeAdjustmentSpec, VolumeAdjustmentsSpec, VolumePeakSpec
...
class CHAP(Frame):
    ...
    def __eq__(self, other):
        ...
        return self.element_id == other.element_id and self.start_time == other.start_time and self.end_time == other.end_time and self.start_offset == other.start_offset and self.end_offset == other.end_offset
    ...
    def _pprint(self):
        ...
        for frame in itervalues(self.sub_frames):
            for line in frame.pprint().splitlines():
                frame_pprint += "\n" + " " * 4 + line
        return u"%s time=%d..%d offset=%d..%d%s" % (self.element_id, self.start_time, self.end_time, self.start_offset, self.end_offset, frame_pprint,)
...
class CTOC(Frame):
    ...
    def __eq__(self, other):
        ...
        return self.element_id == other.element_id and self.flags == other.flags and self.child_element_ids == other.child_element_ids

    def _pprint(self):
        frame_pprint = u""
        if getattr(self, "sub_frames", None):
            frame_pprint += "\n" + "\n".join([" " * 4 + f.pprint() for f in self.sub_frames.values()])
        return u"%s flags=%d child_element_ids=%s%s" % (self.element_id, int(self.flags), u",".join(self.child_element_ids), frame_pprint,)
...
class NumericPartTextFrame(TextFrame):
    ...
    _framespec = [
        EncodingSpec("encoding", default=Encoding.UTF16),
        MultiSpec("text", EncodedNumericPartTextSpec("text"), sep=u"\u0000", default=[]),
    ]
...
class PairedTextFrame(Frame):
    ...
    _framespec = [
        EncodingSpec("encoding", default=Encoding.UTF16),
        MultiSpec("people", EncodedTextSpec("involvement"), EncodedTextSpec("person"), default=[]),
    ]
...
class SYLT(Frame):
    ...
    def __str__(self):
        ...
        return u"\n".join("[{0}{1}]: {2}".format(time, unit, text) for (text, time) in self.text)
...
class APIC(Frame):
    ...
    * data -- raw image data, as a byte string
    ...
    def _pprint(self):
        ...
        return "%s, %s (%s, %d bytes)" % (type_desc, self.desc, self.mime, len(self.data))
...
class LINK(Frame):
    ...
    * data -- further ID information for the frame
    ...
    @property
    def HashKey(self):
        return "%s:%s:%s:%s" % (self.FrameID, self.frameid, self.url, _bytes2key(self.data))
