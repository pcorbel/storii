...
from mutagen._senf import argv, print_
...
class ID3OptionParser(OptionParser):
    def __init__(self):
        ...
        OptionParser.__init__(
            self, version=version, usage="%prog [option(s)] <src> <dst>", description=("Copies ID3 tags from <src> to <dst>. Mutagen-based " "replacement for id3lib's id3cp."),
        )
...
def main(argv):
    ...
    parser.add_option(
        "-v", "--verbose", action="store_true", dest="verbose", help="print out saved tags", default=False,
    )
    parser.add_option(
        "--write-v1", action="store_true", dest="write_v1", default=False, help="write id3v1 tags",
    )
    parser.add_option(
        "-x", "--exclude-tag", metavar="TAG", action="append", dest="x", help="exclude the specified tag", default=[],
    )
    parser.add_option(
        "--merge", action="store_true", help="Copy over frames instead of the whole ID3 tag", default=False,
    )
    ...
    with _sig.block():
        return copy(src, dst, options.merge, options.write_v1, excluded_tags, options.verbose)
