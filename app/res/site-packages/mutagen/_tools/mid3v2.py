...
from mutagen._senf import argv, bytes2fsn, fsn2bytes, fsn2text, fsnative, print_
...
class ID3OptionParser(OptionParser):
    def __init__(self):
        ...
        OptionParser.__init__(
            self, version=version, usage="%prog [OPTION] [FILE]...", description="Mutagen-based replacement for id3lib's id3v2.",
        )
            ...
            for (frame, vlist) in edits.items():
                if frame == "POPM":
                    for value in vlist:
                        ...
                        frame = mutagen.id3.POPM(email=email, rating=int(rating), count=int(count))
                        id3.add(frame)
                elif frame == "APIC":
                    for value in vlist:
                        values = string_split(value, ":")
                        ...
                        frame = mutagen.id3.APIC(encoding=encoding, mime=mime, desc=desc, type=picture_type, data=data)
                    ...
                    for value in vlist:
                        ...
                        frame = mutagen.id3.COMM(encoding=3, text=value, lang=lang, desc=desc)
                        id3.add(frame)
                elif frame == "USLT":
                    for value in vlist:
                        values = string_split(value, ":")
                        if len(values) == 1:
                            ...
                            value = ":".join(values[1:-1])
                            desc, lang = values[0], values[-1]
                        frame = mutagen.id3.USLT(encoding=3, text=value, lang=lang, desc=desc)
                        id3.add(frame)
                elif frame == "UFID":
                    for value in vlist:
                        values = string_split(value, ":")
...
def main(argv):
    ...
    parser.add_option("-q", "--quiet", action="store_false", dest="verbose", help="be quiet (the default)")
    parser.add_option("-e", "--escape", action="store_true", default=False, help="enable interpretation of backslash escapes")
    parser.add_option(
        "-f", "--list-frames", action="callback", callback=list_frames, help="Display all possible frames for ID3v2.3 / ID3v2.4",
    )
    parser.add_option(
        "--list-frames-v2.2", action="callback", callback=list_frames_2_2, help="Display all possible frames for ID3v2.2",
    )
    parser.add_option("-L", "--list-genres", action="callback", callback=list_genres, help="Lists all ID3v1 genres")
    parser.add_option("-l", "--list", action="store_const", dest="action", const="list", help="Lists the tag(s) on the open(s)")
    parser.add_option(
        "--list-raw", action="store_const", dest="action", const="list-raw", help="Lists the tag(s) on the open(s) in Python format",
    )
    parser.add_option("-d", "--delete-v2", action="store_const", dest="action", const="delete-v2", help="Deletes ID3v2 tags")
    parser.add_option("-s", "--delete-v1", action="store_const", dest="action", const="delete-v1", help="Deletes ID3v1 tags")
    parser.add_option(
        "-D", "--delete-all", action="store_const", dest="action", const="delete-v1-v2", help="Deletes ID3v1 and ID3v2 tags",
    )
    parser.add_option(
        "--delete-frames", metavar="FID1,FID2,...", action="store", dest="deletes", default="", help="Delete the given frames",
    )
    parser.add_option(
        "-C", "--convert", action="store_const", dest="action", const="convert", help="Convert tags to ID3v2.4 (any editing will do this)",
    )

    parser.add_option(
        "-a", "--artist", metavar='"ARTIST"', action="callback", help="Set the artist information", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TPE1"), args[2])),
    )
    parser.add_option(
        "-A", "--album", metavar='"ALBUM"', action="callback", help="Set the album title information", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TALB"), args[2])),
    )
    parser.add_option(
        "-t", "--song", metavar='"SONG"', action="callback", help="Set the song title information", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TIT2"), args[2])),
    )
    parser.add_option(
        "-c", "--comment", metavar='"DESCRIPTION":"COMMENT":"LANGUAGE"', action="callback", help="Set the comment information", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--COMM"), args[2])),
    )
    parser.add_option(
        "-p", "--picture", metavar='"FILENAME":"DESCRIPTION":"IMAGE-TYPE":"MIME-TYPE"', action="callback", help="Set the picture", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--APIC"), args[2])),
    )
    parser.add_option(
        "-g", "--genre", metavar='"GENRE"', action="callback", help="Set the genre or genre number", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TCON"), args[2])),
    )
    parser.add_option(
        "-y", "--year", "--date", metavar="YYYY[-MM-DD]", action="callback", help="Set the year/date", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TDRC"), args[2])),
    )
    parser.add_option(
        "-T", "--track", metavar='"num/num"', action="callback", help="Set the track number/(optional) total tracks", type="string", callback=lambda *args: args[3].edits.append((fsnative(u"--TRCK"), args[2])),
    )

    for key, frame in mutagen.id3.Frames.items():
        if issubclass(frame, mutagen.id3.TextFrame) or issubclass(frame, mutagen.id3.UrlFrame) or issubclass(frame, mutagen.id3.POPM) or frame in (mutagen.id3.APIC, mutagen.id3.UFID, mutagen.id3.USLT):
            parser.add_option(
                "--" + key, action="callback", help=SUPPRESS_HELP, type="string", metavar="value", callback=lambda *args: args[3].edits.append(args[1:3]),  # optparse blows up with this
            )
