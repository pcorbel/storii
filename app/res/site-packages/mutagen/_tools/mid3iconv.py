...
from mutagen._senf import argv, fsnative, print_
...
class ID3OptionParser(OptionParser):
    def __init__(self):
        ...
        return OptionParser.__init__(self, version=version, usage="%prog [OPTION] [FILE]...", description=("Mutagen-based replacement the id3iconv utility, " "which converts ID3 tags from legacy encodings " "to Unicode and stores them using the ID3v2 format."),)
...
def main(argv):
    ...
    parser.add_option(
        "-e", "--encoding", metavar="ENCODING", action="store", type="string", dest="encoding", help=("Specify original tag encoding (default is %s)" % (getpreferredencoding())),
    )
    parser.add_option(
        "-p", "--dry-run", action="store_true", dest="noupdate", help="Do not actually modify files",
    )
    parser.add_option(
        "--force-v1", action="store_true", dest="force_v1", help="Use an ID3v1 tag even if an ID3v2 tag is present",
    )
    parser.add_option(
        "--remove-v1", action="store_true", dest="remove_v1", help="Remove v1 tag after processing the files",
    )
    parser.add_option(
        "-q", "--quiet", action="store_const", dest="verbose", const="quiet", help="Only output errors",
    )
    parser.add_option(
        "-d", "--debug", action="store_const", dest="verbose", const="debug", help="Output updated tags",
    )
