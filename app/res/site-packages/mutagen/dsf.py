...
from mutagen._util import MutagenError, cdata, convert_error, loadfile
from mutagen.id3 import ID3
from mutagen.id3._util import ID3NoHeaderError
from mutagen.id3._util import error as ID3Error

from ._compat import cBytesIO, endswith, reraise
...
class DSDChunk(DSFChunk):
    ...
    def pprint(self):
        return u"DSD Chunk (Total file size = %d, " u"Pointer to Metadata chunk = %d)" % (self.total_size, self.offset_metdata_chunk,)
...
class FormatChunk(DSFChunk):
    ...
    def pprint(self):
        return u"fmt Chunk (Channel Type = %d, Channel Num = %d, " u"Sampling Frequency = %d, %.2f seconds)" % (self.channel_type, self.channel_num, self.sampling_frequency, self.length,)
...
class DataChunk(DSFChunk):
    ...
    def pprint(self):
        return u"data Chunk (Chunk Offset = %d, Chunk Size = %d)" % (self.chunk_offset, self.chunk_size)
...
class _DSFID3(ID3):
    ...
    def save(self, filething=None, v2_version=4, v23_sep="/", padding=None):
        ...
        try:
            data = self._prepare_data(fileobj, dsd_header.offset_metdata_chunk, self.size, v2_version, v23_sep, padding)
...
class DSFInfo(StreamInfo):
    ...
    def pprint(self):
        return u"%d channel DSF @ %d bits, %s Hz, %.2f seconds" % (self.channels, self.bits_per_sample, self.sample_rate, self.length,)
