...
from mutagen._util import BitReader, BitReaderError, MutagenError, convert_error, loadfile
...
class AACInfo(StreamInfo):
    ...
    def pprint(self):
        return u"AAC (%s), %d Hz, %.2f seconds, %d channel(s), %d bps" % (self._type, self.sample_rate, self.length, self.channels, self.bitrate,)
...
class AAC(FileType):
    ...
    def score(filename, fileobj, header):
        ...
        s = endswith(filename, ".aac") or endswith(filename, ".adts") or endswith(filename, ".adif")
        s += b"ADIF" in header
        return s
